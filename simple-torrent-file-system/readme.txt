Project Description Peer-to-Peer Systems
...
to the neighbor from which it received the first copy of the search request.
6. As part of consuming a reply, the search initiator, Ck, collects all the replies received until the expiry of the timer, and then displays them as a list of tuples of the form (kwd, file name, comp name). Replies received after the expiry of the timer are ignored.
...
9. If the search terminated without success, then Cm should double the hop-count and reinitiate the process. This should be repeated until there is success, or the hop-count exceeds sixteen (whichever happens earlier). Note that the timer value is a monotonically increasing function of hop-count.
...
You need to add the following features to your Previous implementation:
1. The requesting machine performs the expanding ring search for the requested file until at least five copies of the file have been found, or the entire network has been searched (whichever happens first).
2. The user that made the request can specify whether the file is to be fetched from only one site or from all sites that returned a positive response.
3. If the user wishes to fetch the file from only one site, then the user gets to specify which response tuple he/she is interested in.
4. If the user wishes to fetch the file from all the n sites that returned a positive response then the file should be logically viewed as consisting of n equal sized pieces. One piece should be fetched from each of the responding sites, and then the pieces should be concatenated to constitute the full file.
